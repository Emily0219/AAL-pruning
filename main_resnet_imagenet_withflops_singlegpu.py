...
def main():
    ...
    if args.depth == 18 or args.depth == 34:
        for k in range(len(model.layer1)):
            bnan_params_id += list(map(id, model.layer1[k].bnan1.parameters()))
            for name, param in model.layer1[k].bnan1.named_parameters():
                bnan_params.append(param)
        for k in range(len(model.layer2)):
            bnan_params_id += list(map(id, model.layer2[k].bnan1.parameters()))
            for name, param in model.layer2[k].bnan1.named_parameters():
                bnan_params.append(param)
        for k in range(len(model.layer3)):
            bnan_params_id += list(map(id, model.layer3[k].bnan1.parameters()))
            for name, param in model.layer3[k].bnan1.named_parameters():
                bnan_params.append(param)
        for k in range(len(model.layer4)):
            bnan_params_id += list(map(id, model.layer4[k].bnan1.parameters()))
            for name, param in model.layer4[k].bnan1.named_parameters():
                bnan_params.append(param)
    else:
        for k in range(len(model.layer1)):
            bnan_params_id += list(map(id, model.layer1[k].bnan1.parameters()))
            bnan_params_id += list(map(id, model.layer1[k].bnan2.parameters()))
            for name, param in model.layer1[k].bnan1.named_parameters():
                bnan_params.append(param)
            for name, param in model.layer1[k].bnan2.named_parameters():
                bnan_params.append(param)
        for k in range(len(model.layer2)):
            bnan_params_id += list(map(id, model.layer2[k].bnan1.parameters()))
            bnan_params_id += list(map(id, model.layer2[k].bnan2.parameters()))
            for name, param in model.layer2[k].bnan1.named_parameters():
                bnan_params.append(param)
            for name, param in model.layer2[k].bnan2.named_parameters():
                bnan_params.append(param)
        for k in range(len(model.layer3)):
            bnan_params_id += list(map(id, model.layer3[k].bnan1.parameters()))
            bnan_params_id += list(map(id, model.layer3[k].bnan2.parameters()))
            for name, param in model.layer3[k].bnan1.named_parameters():
                bnan_params.append(param)
            for name, param in model.layer3[k].bnan2.named_parameters():
                bnan_params.append(param)
        for k in range(len(model.layer4)):
            bnan_params_id += list(map(id, model.layer4[k].bnan1.parameters()))
            bnan_params_id += list(map(id, model.layer4[k].bnan2.parameters()))
            for name, param in model.layer4[k].bnan1.named_parameters():
                bnan_params.append(param)
            for name, param in model.layer4[k].bnan2.named_parameters():
                bnan_params.append(param)
