...
def main():
    best_prec1 = 0
    save = args.model + str(args.depth) + '_cifar10_' + datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    ...
    model_config = {'dataset': 'cifar10', 'depth': args.depth}
    ...
    if args.get_small:
        big_path = os.path.join(save_path, "big_model.pth.tar")
        torch.save(model.state_dict(), big_path)

        x = torch.rand(1, 3, 32, 32)
        # output = model.forward(x)
        # print(output)
        small_model, index_layer, bn_value = get_small_model(model.cpu())
        ...
        # FLOPs
        print_model_param_flops(model, [32, 32])
        print_model_param_flops(small_model, [32, 32])
...
def extract_para(big_model):
    ...
    try:
        assert len(item) in [147, 231, 399, 777]   # torch1.3版本, with bnan
        print("state dict length is one of 147, 231, 399, 777")
    ...
    if len(item) == 147:
        small_model = models.resnet20_small(index=kept_index_per_layer, bn_value=bn_value,
                                            cfg=num_for_construct)
    if len(item) == 231:
        small_model = models.resnet32_small(index=kept_index_per_layer, bn_value=bn_value,
                                            cfg=num_for_construct)
    if len(item) == 399:
        small_model = models.resnet56_small(index=kept_index_per_layer, bn_value=bn_value,
                                            cfg=num_for_construct)
    if len(item) == 777:
        small_model = models.resnet110_small(index=kept_index_per_layer, bn_value=bn_value,
                                            cfg=num_for_construct)
    return kept_index_per_layer, pruned_index_per_layer, block_flag, small_model, bn_value
...
def get_small_model(big_model):
    indice_dict, pruned_index_per_layer, block_flag, small_model, bn_value = extract_para(big_model)
    ...
    for index, [key, value] in enumerate(big_state_dict.items()):
        ...
        if flag_conv_ex_down or flag_down:
            if key == 'conv1.weight':   # not prune the first conv layer
    ...
    return small_model, indice_dict, bn_value
