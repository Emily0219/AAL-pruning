...
def print_model_param_flops(model=None, input_res=[224, 224], multiply_adds=True):
    ...
    import logging
    logging.info('  + Number of FLOPs: %.6fG' % (total_flops / 1e9))
    logging.info('  + Number of conv  FLOPs: %.6fG' % (conv_flops / 1e9))
    print('  + Number of FLOPs: %.6fG' % (total_flops / 1e9))
    print('  + Number of FLOPs: %.6fG' % (conv_flops / 1e9))
